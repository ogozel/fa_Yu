% Slightly modified from original by OG to also output the optimal
% dimensionality based on the cross-validated prediction error (optDimPE)
% and based on the cross-validated log likelihood (optDimLL)

function [dim, optDimPE, optDimLL] = crossvalidate_fa_OG(X, varargin)
%
...
%   rand('state', 0); %%% OG: this line was commented
%   rng('shuffle')  %%% OG: this line was not commented, but gave an error when running 'example.m'
  X = X(:, randperm(N));
  ...
  %% Compute optimal dimensionality
  % OG: we do this no matter if we show plots or not
  sumPE = [dim.sumPE];
  istarPE = find(sumPE == min(sumPE));
  optDimPE = zDimList(istarPE);
  
  sumLL = [dim.sumLL];
  istarLL = find(sumLL == max(sumLL));
  optDimLL = zDimList(istarLL);
  ...
  if showPlots
    ...
    hold on;
    plot(optDimPE, sumPE(istarPE), '*', 'markersize', 5);
    ...
    hold on;
    plot(optDimLL, sumLL(istarLL), '*', 'markersize', 5);
